...
<body>
    ...
    <div class="pos">
     ...
     <h2>{{ $program->NameLong }}</h2>
            ...
            <div id="  w-100" style="margin-left:-50px;height:100vh">
                @php
                    $counter = 1;
                    $cc = 0;
                @endphp
                @foreach ($program->project as $pro_key => $project)
                    ...
                    <table border="1" id="report_table"
                        class="table table-responsive custom-table fullscreen_table tdf-table-primary mx-0 @if ($count == 0) d-none @endif"
                        style="width:100%;height:100vh;margin-top:50px">
                        <thead class="bg-red" style="background:#2767a0;">
                            <tr>
                                <th colspan="{{ 7 + 12 + 4 + 4 }}" class="text-center"
                                    style="font-size: 32px !important;padding10px;">
                            ...
                            <tr>
                                ...
                                <th colspan="2">Project status as of the end of FY 2078/79</th>
                                        ...
                                        @if ($mile->timeline)
                                            @foreach ($mile->timeline->timeline ?? [] as $timeline_month => $time)
                                                @if (in_array($timeline_month, $quaterfilter->showMonths()))
                                                    @if ($mile->is_text == 'yes')
                                                        <td
                                                            style="{{ $key2 % 2 == 0 ? 'background: aliceblue;' : '' }}">
                                                            {{ $time }}
                                                        </td>
                                                    @else
                                                        <td
