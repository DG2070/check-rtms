        ...
        <table border="1" class="table table-responsive custom-table tdf-table-primary"
            ...
            <thead>
                <tr>
                    ...
                    <th>Total Financial Progress</th>
                    <th>Total Physical Target</th>
                    <th>Total Physical Progress</th>
                </tr>
            </thead>
            @foreach ($project->projectActivity ?? [] as $key => $pro_act)
                @foreach ($pro_act->milestone as $key2 => $mile)
            ...
            <thead>
                ...
                <tr>
                    @if ($quaterfilter->firstQuaterCount() > 0)
                        <th colspan="{{ $quaterfilter->firstQuaterCount() }}">1st </th>
                    @endif
                    @if ($quaterfilter->secondQuaterCount() > 0)
                        <th colspan="{{ $quaterfilter->secondQuaterCount() }}">2nd</th>
                    @endif
                    @if ($quaterfilter->thirdQuaterCount() > 0)
                        <th colspan="{{ $quaterfilter->thirdQuaterCount() }}">3rd</th>
                    @endif
                    @if ($quaterfilter->fourthQuaterCount() > 0)
                        <th colspan="{{ $quaterfilter->fourthQuaterCount() }}">4th</th>
                    @endif
                    <th rowspan="2">Achived</th>
                    <th rowspan="2">Not <br> Achived</th>
                </tr>
                ...
                @foreach ($pro_act->milestone as $key2 => $mile)
                    ...
                    <tbody @if ($key2 / 2 == 0) style="page-break-before: always" @endif>
                        ...
                        <tr @if ($key == 0 && $key2 == 0) style="border-top:1.5px solid #2969a2 !important" @endif>
                            ...
                            <td rowspan="{{ $mileRows }}"style="text-align:left;max-width:450px;">
                                {{ $mile->performance_indicator }}</td>

                            <td rowspan="{{ $mileRows }}" style="text-align:left;max-width:450px;">
                                {{ $mile->remark ?? '' }}</td>

                            <td rowspan="{{ $mileRows }}" style="text-align: center;">
                                @if (isset($project->projectReview->target[$mile->id]) && $project->projectReview->target[$mile->id] == 'achived')
                                    <div
                                        style="font-family: DejaVu Sans, sans-serif; border: 2px solid #000; border-radius: 50%; width: 20px; height: 20px; display: inline-block; text-align: center; line-height: 20px;padding:12px;">
                                        ✔
                                    </div>
                                @endif
                            </td>
                            ...
                            <td rowspan="{{ $mileRows }}" style="text-align: center;">
                                @if (isset($project->projectReview->target[$mile->id]) && $project->projectReview->target[$mile->id] == 'not_achived')
                                    <div
                                        style="font-family: DejaVu Sans, sans-serif; border: 2px solid #000; border-radius: 50%; width: 20px; height: 20px; display: inline-block; text-align: center; line-height: 20px;padding:12px;">
                                        ✔
                                    </div>
                            ...
                            @if ($mile->timeline)
                                @foreach ($mile->timeline->timeline ?? [] as $timeline_month => $time)
                                    @if (in_array($timeline_month, $quaterfilter->showMonths()))
                                        ...
                                        @else
                                            <td style="{{ $key2 % 2 == 0 ? 'background: aliceblue;' : '' }}">
                                                @if (isset($mile->timeline->progress_input_data[$loop->index + 1]) &&
                                                        $mile->timeline->progress_input_data[$loop->index + 1] == 1)
                                                    <div
                                                        style="font-family: DejaVu Sans, sans-serif; border: 2px solid #000; border-radius: 50%; width: 20px; height: 20px; display: inline-block; text-align: center; line-height: 20px;padding:12px;">
                                                        ✔
                                                    </div>
